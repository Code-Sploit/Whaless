...
static const uint64_t ZOBRIST_RANDOMS[781];

static inline int zobrist_ptp(struct __piece __piece)
{
...
ZobristHash zobrist_hash_state(struct __game_state *__state)
{
    ...
    static const int WHITE_CASTLE_RIGHT = 768;
    static const int WHITE_CASTLE_LEFT = 769;
    static const int BLACK_CASTLE_RIGHT = 770;
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        {
            struct __board_pos __pos = __state->__piece_list_white[i];
            struct __piece __p = get_piece(__state, __pos);

            __hash ^= ZOBRIST_RANDOMS[PIECES + 64 * zobrist_ptp(__p) + 8 * (7 - __pos.__rank) + __pos.__file];
        }

        if (!boardpos_eq(__state->__piece_list_black[i], NULL_BOARDPOS))
        {
            struct __board_pos __pos = __state->__piece_list_black[i];
            struct __piece __p = get_piece(__state, __pos);
    ...
    if (__state->__is_turn_white)
    {
        __hash ^= ZOBRIST_RANDOMS[WHITE_TO_MOVE];
    }

    if (__state->__white_castle_right)    __hash ^= ZOBRIST_RANDOMS[WHITE_CASTLE_RIGHT];
    if (__state->__white_castle_left)     __hash ^= ZOBRIST_RANDOMS[WHITE_CASTLE_LEFT];
    if (__state->__black_castle_right)    __hash ^= ZOBRIST_RANDOMS[BLACK_CASTLE_RIGHT];
    if (__state->__black_castle_left)     __hash ^= ZOBRIST_RANDOMS[BLACK_CASTLE_LEFT];

    enum __player __to_move = __state->__is_turn_white ? PLAYER_WHITE : PLAYER_BLACK;

    int __enp_file = get_enpassant_target_file(__state, __to_move);
    int __enp_rank = __state->__is_turn_white ? 3 : 4;

    if (__enp_file != -1)
    {
        bool __got_pawn = false;

        if (__enp_file != 7)
        {
            struct __piece __p = get_piece(__state, BoardPos(__enp_file + 1, __enp_rank));

            if (__p.__type != PIECE_EMPTY && __p.__player == __to_move)
            {
                __hash ^= ZOBRIST_RANDOMS[ENPASSANT + __enp_file];

                __got_pawn = true;
            }
        }

        if (!__got_pawn && __enp_file != 0)
        {
            struct __piece __p = get_piece(__state, BoardPos(__enp_file - 1, __enp_rank));
...
// polyglot format zobrist hashes
// https://www.chessprogramming.org/Zobrist_Hashing
// http://hgm.nubati.net/book_format.html
// first 768 keys for each piece at each square,
// next four keys indicate castling rights (white right, white left, black right, black left)
// next eight keys indicate enpassant files (not target files, only if the player to move has a pawn next to an enemy pawn that just double pushed)
// next & last key indicates if the side to move is white
static const uint64_t ZOBRIST_RANDOMS[781] = 
{
